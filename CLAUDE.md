...
A hash-allowlisted inline boot script in `index.html` runs before first paint: it applies the saved dark theme and adds `.booting` on `<html>` (hiding the prerendered landing) when the visitor is signed in or on a deep link, so refreshes don't flash the landing snapshot; `main.jsx` removes the class after render. **If you change that script by even one character, recompute its CSP hash** (`printf %s '<script body>' | openssl dgst -sha256 -binary | openssl base64`) and update the `sha256-…` entry in the CSP meta tag. The service worker's `navigateFallbackDenylist` (vite.config.js) keeps browser navigations to `/robots.txt` and `/sitemap.xml` from being hijacked by the SPA fallback — curl/crawlers were never affected, but the SW made them look missing in a browser. The site is served at `https://tunebox.net` (custom domain from `public/CNAME`). No manual deploy step — just merge to `main`.

### Portland Collection page — `/portland-collection/`

A standalone static SEO/traffic page (like `privacy.html`, outside the SPA) indexing all tunes of *The Portland Collection* books. `scripts/build-portland-collection.mjs` generates `public/portland-collection/{index.html,tunes-abc.json}` from the checked-in dataset `scripts/portland-collection/tunes.json` (produced by a scraping/matching pipeline that lives outside this repo — only results are checked in). Regenerate manually with `node scripts/build-portland-collection.mjs` and commit the output; the full tune list is static HTML so crawlers see every name. `public/portland-collection/app.js` (hand-written) adds search/filter/sort and lazy-loads abcjs from `public/abcjs/` (gitignored, copied on postinstall) plus the ABC JSON on first "Sheet music" click; audio uses the self-hosted `/soundfonts/`. The page has its own CSP meta tag and GoatCounter snippet, is excluded from the SW precache and `navigateFallback` (vite.config.js), and is listed in `sitemap.xml`. Deliberately **not linked from anywhere in the app** — it links out to tunebox.net, never the reverse.
...
- **`src/lib/ocr.js`** + **`src/lib/pdf-import.js`** + **`src/lib/ocr-title.js`** + **`src/components/bulk/file-import.jsx`** — "File import" on the add-tune page: each uploaded picture/PDF becomes a tune with the file as its `main_source` sheet_music attachment. Images are OCR'd with tesseract.js; PDFs go through pdf.js — text layer first (exact titles from e.g. Session print-PDFs), rendered-page OCR only when there is no text layer; pdf.js also renders the row thumbnails. Both libraries are lazy: nothing loads until the first file is picked. CSP interplay: tesseract needs `workerBlobURL: false` (its default blob-wrapped worker violates the CSP) and `'wasm-unsafe-eval'` in script-src; pdf.js needs `isEvalSupported: false`. All runtime assets (~15MB: tesseract worker/simd-lstm core/eng traineddata, pdf.js worker/wasm decoders/fallback fonts) are served from `public/{tesseract,pdfjs}/` (gitignored), copied out of node_modules by `scripts/copy-vendor-assets.mjs` on postinstall so versions track the installed packages; they're excluded from the SW precache (vite.config.js `globIgnores`). `ocr-title.js` guesses the tune title (and sometimes type) from OCR/text-layer lines — tuned for precision over recall on real scans/photos; returns an empty title rather than staff-notation garbage, and the UI never overwrites user-typed text with recognition results.
